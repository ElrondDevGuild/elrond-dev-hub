...
export default class CreateApplicationAction extends BaseAction {
  async handle(req: ApiRequest): Promise<ApiResponse> {
    ...
    if (bounty.owner_id === user?.id) {
      return new ApiResponse().status(403).body({ message: "You cannot apply to your own bounty" });
    }
    if (bounty.status !== "open") {
      return new ApiResponse().status(403).body({ message: "This bounty is not open for applications" });
    }
    if (!data.terms) {
      return new ApiResponse().status(422).body({ message: "You must accept the terms and conditions" });
    }

    const existingApplication = await this.getUserApplication(user?.id as string, bountyId as string);
    if (existingApplication) {
      return new ApiResponse().status(422).body({ message: "You have already applied to this bounty" });
    }
