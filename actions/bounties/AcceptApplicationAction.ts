...
export default class AcceptApplicationAction extends ApplicationStatusBaseAction {
    async handle(req: ApiRequest): Promise<ApiResponse> {
        ...
        const bountyApplication = await repository.updateApprovalStatus(applicationId as string, "accepted");
        if (bounty!.status === "open") {
            await bountyRepository.updateStatus(bounty!.id, "work_started");
        }
