      ...
      {({ open }) => (
        <>
          <div className="mt-1 relative">
            <Listbox.Button className="text-theme-text dark:text-theme-text-dark font-medium bg-white dark:bg-secondary-dark-lighter relative w-full border border-theme-border dark:border-theme-border-dark pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
              ...
              <Listbox.Options className="absolute z-10 mt-1 w-full bg-white dark:bg-secondary-dark-lighter shadow-lg max-h-60 py-1 ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none text-sm">
                {options.map((option) => (
                  ...
                  >
                    {({ selected, active }) => (
                      <>
                        <div className={classNames(selected ? "font-semibold" : "font-normal", "flex items-center")}>
                          {option.icon && (
