...
export default function UserApplicationsList({user}: { user: User }) {
    ...
    const loadItems = async (page: number) => {
        ...
        setLoading(true);
        try {
            const {data: {applications, count}} = await api.get("bounties/applications", {
                params: {page, page_size: 5, owner_id: user.id}
            });
            setApplications(
                (oldApplications) => page === 0 ? applications : [...oldApplications, ...applications]
            );
            setPage(page);
            ...
            <ul role="list" className="">
                {applications.map((application: BountyApplication) => (
                    <li key={application.id}
                        ...
                        <Link
                            href={bountyPath(application.bounty_id)}
                        >
                        ...
                        <div className="flex items-center">
                            <span className={classNames(
                                ...
                                {application.approval_status === "rejected" ? "Rejected by client" : null}
                                {
                                    ...
                                    && application.work_status !== "pending"
                                        ? ucFirst(application.work_status.split("_").join(" "))
                                        : null
                                }
                            </span>
                        </div>
