...
const BountyCard: React.FC<BountyCardProps> = ({
  ...
  // Determine if the bounty is effectively ended for display purposes
  const isEffectivelyEnded = bounty.hasDeadlineEnded && ['open', 'active', 'in_progress'].includes(bounty.status);
  // Condition to apply dimming effect to the card
  const applyCardDimmingEffect = bounty.status === 'closed' || bounty.status === 'pending' || bounty.status === 'canceled' || isEffectivelyEnded;
  ...
  const getStatusTag = () => {
    ...
    if (isEffectivelyEnded) {
      displayStatus = 'ended_by_deadline'; // Use a special key for this display case
    }
    ...
    switch (displayStatus) {
      ...
      case "pending":
        ...
        text = "Pending";
        colorClasses =
          "bg-gray-100 dark:bg-gray-900/20 text-gray-700 dark:text-gray-400 border border-gray-200 dark:border-gray-800";
        break;
      case "completed":
        icon = <BsCheck2Circle className="mr-1.5" />;
        text = "Completed";
      ...
      case "ended_by_deadline": 
        ...
        text = "Ended"; 
        colorClasses =
          "bg-gray-100 dark:bg-gray-900/20 text-gray-700 dark:text-gray-400 border border-gray-200 dark:border-gray-800"; // Grey styling
        break;
      case "closed": 
      default: 
        icon = <BsExclamationTriangle className="mr-1.5" />;
    ...
    return (
      <span
        className={`inline-flex items-center px-2.5 py-1 text-xs font-medium rounded-md ${colorClasses}`}
      >
        {icon} {text}
      </span>
    );
  ...
  const deadline = bounty.deadlineAt
    ? formatDeadline(bounty.deadlineAt)
    : formatDeadline(null);
  ...
  if (viewMode === "list") {
    return (
      <div
        className={`relative bg-white dark:bg-secondary-dark rounded-xl overflow-hidden shadow-sm border border-theme-border dark:border-theme-border-dark ${
          applyCardDimmingEffect ? "opacity-60" : "opacity-100"
        }`}
      >
              ...
              {deadline.hasDeadline && !deadline.urgent && (
                <span
                  className={`flex items-center ${
                    deadline.isEnded ? "text-red-500 dark:text-red-400" : ""
                  }`}
                >
  ...
  return (
    <div
      className={`relative bg-white dark:bg-secondary-dark rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-shadow duration-200 border border-theme-border dark:border-theme-border-dark flex flex-col h-full group ${
        applyCardDimmingEffect ? "opacity-60" : ""
      }`}
    >
      {/* Optional: Add a small overlay if effectively closed, to make it more visually distinct */}
      {applyCardDimmingEffect && (
        <div className="absolute inset-0 bg-gray-500/10 dark:bg-gray-800/10 z-0"></div>
      )}
      ...
      <div className="p-5 border-t border-theme-border dark:border-theme-border-dark mt-auto z-50">
